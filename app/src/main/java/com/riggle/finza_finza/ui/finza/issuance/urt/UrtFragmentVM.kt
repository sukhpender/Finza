...
@HiltViewModel
class UrtFragmentVM @Inject constructor(private val baseRepo: BaseRepo): BaseViewModel() {

    val obrListing = SingleRequestEvent<UrtResponseModel>()
    fun getList(token:String,month: String,year: String) {
        Coroutines.io {
            obrListing.postValue(Resource.loading(null))
            try {
                baseRepo.urtListing(token,month,year).let {
                    if (it.isSuccessful) {
                        it.body()?.let { results ->
