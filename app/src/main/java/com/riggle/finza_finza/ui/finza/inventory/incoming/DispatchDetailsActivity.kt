...
import com.riggle.finza_finza.databinding.ActivityDispatchDetailsBinding
...
import com.riggle.finza_finza.ui.finza.inventory.forwarded.ForwardedDetailsActivity.Companion
...
@AndroidEntryPoint
class DispatchDetailsActivity : BaseActivity<ActivityDispatchDetailsBinding>(),
    VerticalPagination.VerticalScrollListener {

    private val viewModel: DispatchDetailsActivityVM by viewModels()
    ...
    companion object {
        fun newIntent(activity: Activity): Intent {
            val intent = Intent(activity, DispatchDetailsActivity::class.java)
    ...
    private fun initView() {
        ...
        viewModel.getInventory1(
            sharedPrefManager.getToken().toString(), "1", currentPage, userID.toInt()
        )
            ...
            when (it?.status) {
                ...
                Status.SUCCESS -> {
                    ...
                    if (it.data != null) {
                        ...
                        if (it.data.data.data.isNotEmpty()) {
                            binding.tvShowingTags.visibility = View.VISIBLE
                            binding.llDispatched.visibility = View.VISIBLE
                            ...
                            binding.tvShowingTags.visibility = View.GONE
                            binding.llDispatched.visibility = View.GONE
        ...
        viewModel.obrAcceptReject.observe(this) {
            when (it?.status) {
                Status.LOADING -> {
                    showHideLoader(true)
                }
                    ...
                    if (adapter.list.size != 0) {
                        for (i in 0 until adapter.list.size) {
                            if (adapter.list[i].isSelected) {
                                selectedItems.add(adapter.list[i].inventory_id)}
                        }
                    }
