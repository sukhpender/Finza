...
class DownloadRCActivityVM @Inject constructor(private val baseRepo: BaseRepo) : BaseViewModel() {
    ...
    val obrDownloadList = SingleRequestEvent<RcDownloadedResponseModel>()
    fun downloadList(token: String,page: Int) {
        Coroutines.io {
            obrDownloadList.postValue(Resource.loading(null))
            try {
                baseRepo.downloadRcList(token,page).let {
                    if (it.isSuccessful) {
                        it.body()?.let { results ->
                            obrDownloadList.postValue(Resource.success(results, "Success"))
                        }
                    } else {
                        obrDownloadList.postValue(
                            Resource.error(
                ...
                parseException(e.cause)?.let {
                    obrDownloadList.postValue(Resource.error(null, it))
                }
            }
        }
    }
