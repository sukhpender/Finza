package com.riggle.plug.ui.myBrands.coupons

import com.riggle.plug.data.api.BaseRepo
import com.riggle.plug.data.model.BrandOfferResponseModel
import com.riggle.plug.data.model.BrandOfferResult
import com.riggle.plug.data.model.ProductResult
import com.riggle.plug.ui.base.BaseViewModel
import com.riggle.plug.utils.Coroutines
import com.riggle.plug.utils.Resource
import com.riggle.plug.utils.event.SingleRequestEvent
import com.riggle.plug.utils.parseException
import dagger.hilt.android.lifecycle.HiltViewModel
import javax.inject.Inject

@HiltViewModel
class CouponsFragmentVM @Inject constructor(private val baseRepo: BaseRepo): BaseViewModel(){

    val obrBrandOffers: SingleRequestEvent<List<BrandOfferResult>> = SingleRequestEvent()

    fun getBrandOffersList(
        header: String,query:Map<String, String>
    ) {
        Coroutines.io {
            obrBrandOffers.postValue(Resource.loading(null))
            try {
                baseRepo.getBrandOffersList(header,query).let {
                    if (it.isSuccessful) {
                        it.body()?.results?.let { results ->
                            obrBrandOffers.postValue(Resource.success(results, "Success"))
                        }
                    } else {
                        obrBrandOffers.postValue(
                            Resource.error(
                                null, handleErrorResponse(it.errorBody(),it.code())
                            )
                        )
                    }
                }
            } catch (e: Exception) {
                parseException(e.cause)?.let {
                    obrBrandOffers.postValue(Resource.error(null, it))
                }
            }
        }
    }
}